...
% ;         param.difficult: set this keyword to the number of
...
% ;     'mem' will yield the highest fidelity .
% ; 
% ; SIDE EFFECTS:
...
% ; 10/10  BPB Added fields 'difficult' and 'excessive' to parameter
% ;     structure. Added support for param structure fields that are not
...
if nargin == 2
    ...
    quiet = 0;
    difficult = 5.e+4;
    excessive = 1; % exit if combinations > 10*difficult
else
    if isfield(param,'mem'); memory_b = param.mem; else memory_b = 0; end
    if isfield(param,'good'); goodenough = param.good; 
        else goodenough = 0;end
    if isfield(param,'dim'); dim = param.dim; else dim = dd-1; end
    if isfield(param,'quiet'); quiet = param.quiet; else quiet = 0; end
    if isfield(param,'difficult'); difficult = param.difficult; 
...
w = find(st > 0);
z = length(w);    % number of unique times in position list
z = z +1;
if isempty(w)
...
%                end
    ...
    if (ispan == zspan) | (i == z)
        ...
        if quiet~=1
            disp([num2str(i), ' of ' ,...
                num2str(z), ' done.  Tracking  ',num2str(ntrk),...
                ' particles  ', num2str(n),' tracks total']);
        end
