...
for i = 1:imcnt
    ...
    if isempty(t_pkfind)
        ...
        if t_npeaks < 3
            ...
            if isfield(sptpara,'cell_num') && sptpara.cell_num==1% checking if it is a single
                %cell and then we can just do delaunay
            ...
            ulen = unique(len);
            hlenbar = mean(ulen)/2;
            im.planeAttr(i).iparticledis_min(1) = min(ulen);
            im.planeAttr(i).iparticledis(1) = mean(ulen);
            im.planeAttr(i).iparticledis(2) = std(ulen);
            im.planeAttr(i).densityindex(1) = sptpara.max_disp/hlenbar;
            
            % Warn if average distance is less than max disp skip this if
            % using spt_interactive you aren't going to use the warning
            % anyway
            elseif isfield(sptpara,'cell_num') && sptpara.cell_num>1% if more than one cell then
                % we need to use alphashape and determine the interparticle
                % distance in each cell
                X=[];
                X=[t_cen(:,1),t_cen(:,2)];
            [V,S] = spt_alpha(X,15,1);
            if isempty(S.tri)==1
                % if number of particle per shape is less than 3 then using upto 
                % 30% quantile of the delaunay to determine the average
            ...
            for j = 1:ntri
                ...
                y= t_cen(TRI(j,:),2);
                dy = y - circshift(y,-1);
                len1(ii:ii+2)=sqrt(dx.^2+dy.^2);
                ii=ii+3;
            end
            ulen = unique(len1);  
            quant=40/quantile(ulen,0.95);% is average cell size
            ulen(find(ulen<quantile(ulen,quant))); % this 0.3 number can be changed to
            % match the distribution of interparticle distance
            else
            TRI1 = S.tri;
            if numel(TRI1)==3
                TRI1=TRI1';% special case when there is only one triangle
            end
            Edg_alpha = unique(sort([TRI1(:,2) TRI1(:,1); TRI1(:,3) TRI1(:,2); TRI1(:,1) TRI1(:,3)],2),'rows');
            % edges included in the alpha shape
            X=[t_cen(:,1),t_cen(:,2)];
            for E =1:size(Edg_alpha,1)
    distx(E)=(X(Edg_alpha(E,2),1)-X(Edg_alpha(E,1),1))^2;
    disty(E)=(X(Edg_alpha(E,2),2)-X(Edg_alpha(E,1),2))^2;
    dist(E)=sqrt(distx(E)+disty(E));
            end
            ulen = unique(dist);
            
            end
            hlenbar = mean(ulen)/2;
            im.planeAttr(i).iparticledis(1) = double(mean(ulen));
            im.planeAttr(i).iparticledis(2) = double(std(ulen));
            im.planeAttr(i).densityindex = sptpara.max_disp/hlenbar;
            end
